...
function M.preview_item_wrapper(resolver, opts)
  ...
  return function(item)
    ...
    if type(preview) == 'table' and not vim.islist(preview) then
    ...
    if type(preview_opts.data) == 'string' or vim.islist(preview_opts.data) then
  ...
  local function create_listview()
    ...
    listview = M.new_list_view({
      ...
      on_confirm = function(item)
        ...
        -- vim.ui.select() contract: on_choice(item, idx). Pass the full row
        -- (it carries .value plus sibling fields like .action/.ctx that LSP
        -- code-action callbacks index) along with the 1-based index.
        return on_choice(item, item.idx)
      end,
