...
-- Авто-сигнатура LSP на `(` и `,` (вместо lsp_signature.nvim, руками).
-- Встроенный vim.lsp.buf.signature_help сам по себе по скобкам не вызывается,
-- поэтому дёргаем его из InsertCharPre — но только в контексте вызова,
-- иначе gopls вернёт пустоту с плавающим "No signature help available":
--   `(`: срабатываем, если перед курсором слово/`)/`]` (имя функции,
--       метод, generic, чейнинг) — отсекает `if (`, `import (`, `.(T`;
--   `,`: срабатываем, если на строке есть незакрытая `(` (внутри аргументов).
-- Плюс общий veto: не стреляем внутри комментариев и строк (treesitter).
local function sig_in_comment_or_string()
...
local function sig_should_trigger(char)
...
vim.api.nvim_create_autocmd("InsertCharPre", {
	callback = function()
		...
		if not sig_should_trigger(char) then
			return
		end
		-- После вставки символа: позиция для gopls уже корректная.
