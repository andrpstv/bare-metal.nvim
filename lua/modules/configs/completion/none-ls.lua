return function()
	...
	require("completion.mason-none-ls").setup()
	...
	vim.api.nvim_create_user_command("NoneLsToggle", function(opts)
		if vim.tbl_contains(_gen_completion(), opts.args) then
			null_ls.toggle({ name = opts.args })
		else
			vim.notify(
				string.format("[none-ls] Unable to find any registered source named [%s].", opts.args),
				vim.log.levels.ERROR,
				{ title = "none-ls Internal Error" }
			)
		end
	end, {
		nargs = 1,
